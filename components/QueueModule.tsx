...
export const QueueModule: React.FC<QueueModuleProps> = ({
  ...
  return (
    <div id="queue-module" className="w-full h-full">
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-5 shadow-2xl backdrop-blur-sm relative overflow-visible h-full flex flex-col">
        ...
        <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-sm font-bold text-[var(--text-main)] uppercase tracking-wider">Bad Decisions Analyzed</h2>
        </div>

        {/* List of expenses - Reduced Spacing */}
        <div className="space-y-2 mb-4 flex-grow">
          {expenses.map((expense) => (
            <div
              key={expense.id}
              className="group flex flex-col md:flex-row items-start md:items-end gap-3 p-3 rounded-2xl bg-[var(--bg-hover)] border border-[var(--border)] hover:border-[var(--primary)] transition-all duration-200"
            >
              ...
              <div className="flex items-center justify-between w-full md:w-auto md:pb-3 gap-3">
                 <div className="flex flex-col justify-end h-full pt-1">
                    <label className="relative inline-flex items-center cursor-pointer group">
                      ...
                      />
                      {/* Updated Toggle Styling: Solid Black Track, Solid Pink Thumb, No Border */}
                      <div className="w-10 h-6 bg-[var(--bg-input)] peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-[var(--text-muted)] after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-[var(--bg-input)] peer-checked:after:bg-[var(--primary)] transition-colors border border-[var(--border)]"></div>
                      <span className="ml-2 text-[10px] text-[var(--text-muted)] font-medium uppercase tracking-wider group-hover:text-[var(--text-main)] transition-colors">{expense.isWant ? 'WANT' : 'NEED'}</span>
                    </label>
                 </div>

                 <button
                    onClick={() => onRemove(expense.id)}
                    className="p-2.5 text-[var(--text-muted)] hover:text-red-400 transition-colors rounded-lg hover:bg-red-900/10"
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-auto pt-2">
          <button
            onClick={onAdd}
            className="flex-none sm:w-40 py-3 border border-[var(--border)] bg-[var(--bg-input)] rounded-xl text-[var(--text-muted)] font-medium hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] transition-all flex items-center justify-center gap-2 text-xs"
          >
          ...
          <button
            onClick={onAnalyze}
            className="flex-1 py-3 bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white rounded-xl font-bold shadow-lg shadow-[var(--primary)]/20 transition-all flex items-center justify-center gap-2 active:scale-[0.99] text-sm"
          >
